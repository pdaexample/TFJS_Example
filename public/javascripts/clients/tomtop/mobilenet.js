...
var MobileNet = {
  ...
  getTopResult: async function (logits) {
    ...
    const finalResult = [];
    /* TODO：使用结果阵列引入商务逻辑 eq.*/
    for (let i = 0; i < valuesAndIndices.length; i++) {
      finalResult.push({
        name: CATEGORIES[valuesAndIndices[i].index].name,
        probability: valuesAndIndices[i].value,
        presnet: CATEGORIES[valuesAndIndices[i].index].keyword,
        index: valuesAndIndices[i].index,
        link : CATEGORIES[valuesAndIndices[i].index].link
      })
    }
    ...
    if (redirect) {
      window.location.href = finalResult[0].link;
    }
  }
}
